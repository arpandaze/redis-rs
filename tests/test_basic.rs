...
use redis::{
    Commands, ConnectionInfo, ConnectionLike, ControlFlow, ErrorKind, ExpiryOption, PubSubCommands,
    RedisResult,
};
...
fn test_getex() {
    ...
    let ret_value = con.get_ex::<_, usize>("foo", ExpiryOption::EX(1)).unwrap();
    ...
    let ret_value = con.get_ex::<_, usize>("foo", ExpiryOption::PERSIST).unwrap();
